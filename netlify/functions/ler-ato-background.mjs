...
ESCOPO: considere SOMENTE as deliberações do corpo do instrumento, que vêm ANTES do capítulo/título de consolidação do contrato social. O texto do contrato consolidado (cláusulas reproduzidas depois da consolidação) NÃO gera deliberações e NÃO deve gerar observações; use-o apenas, se necessário, para completar dados cadastrais (CNPJ, NIRE, endereço, administradores). Em "observacoes", aponte apenas o que afeta o registro das deliberações.`;
...
async function gravar(token, anon, jobId, obj) {
  ...
  /* grava UMA vez só (insert). Com a chave de serviço, não depende das regras de acesso do Storage. */
